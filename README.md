...
### Deploy a relay to Vercel (experimental)
...
The template provisions Neon Postgres and Upstash Redis through Vercel and uses
Tigris (or another conforming S3-compatible store) for media and Git objects.
Vercel Blob is not S3-compatible and cannot safely replace Buzz's Git store
without a new storage backend. See the [Vercel deployment guide](deploy/vercel/README.md)
for the required values and the WebSocket, payload-size, and storage caveats.
